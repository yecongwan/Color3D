...
class ConvNeXt(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.head_cls(x)
        stage1 = self.stages[0](self.downsample_layers[0](x))
        norm_layer = getattr(self, f'norm{0}')
        norm_layer(stage1)
        stage2 = self.stages[1](self.downsample_layers[1](stage1))
        norm_layer = getattr(self, f'norm{1}')
        norm_layer(stage2)
        stage3 = self.stages[2](self.downsample_layers[2](stage2))
        norm_layer = getattr(self, f'norm{2}')
        norm_layer(stage3)
        stage4= self.stages[3](self.downsample_layers[3](stage3))
